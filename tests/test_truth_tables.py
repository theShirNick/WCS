...
class TestTruthTables(unittest.TestCase):
    ...
    def test_str(self):
        R = getRandomAtom()
        self.assertEqual(str(equivalence(negation(T), conjunction(F, R))), f"¬⊤↔⊥∧{R.name}_⊤")
        R1 = getRandomAtom()
        R2 = getRandomAtom()
        self.assertEqual(str(implication(negation(disjunction(T, R1)), R2)), f"¬(⊤∨{R1.name})→{R2.name}_⊤")
        self.assertEqual(str(reverse(R2, negation(disjunction(T, R1)))), f"{R2.name}←¬(⊤∨{R1.name})_⊤")
        self.assertEqual(str(disjunction(F,negation(U))), f"⊥∨¬U_U")
        self.assertEqual(str(implication(F,conjunction(U,T))), f"⊥→U∧⊤_⊤")
        self.assertEqual(str(disjunction(F,negation(conjunction(U,T)))), f"⊥∨¬(U∧⊤)_U")
        self.assertEqual(str(disjunction(F,conjunction(disjunction(U,T), F))), f"⊥∨(U∨⊤)∧⊥_⊥")