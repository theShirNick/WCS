...
def find_vars_and_consts(clauses, variables, consts):
    '''
    Populate the list of vars and consts based on token type
    '''
...
def ground(clauses, variables, consts):
    '''
    Replace each var with each const
    '''
    ground_clauses = set()
    ...
    # Cteate all combinations of consts equal to the number of vars 
    substitutions = list(itertools.product(consts, repeat = len(variables))) 

    # Substitute vars with consts from each combinations
